...
/**
 * @author: zhou
 * @Description: Excel文件操作工具类，包括读、写、合并等功能
...
//%%%%%%%%-------字段部分 结束----------%%%%%%%%%
     ...
     * @throws IOException
     */
    public List<Row> readExcel(InputStream inputStream, String fileName) throws IOException {
    ...
    public List<Row> readExcelByIndex(InputStream inputStream, String fileName, int indexId) throws IOException {
        List<Row> list = new ArrayList<Row>();
        if (this.inputStream != null) {
            inputStream = this.inputStream;
        }
        ...
        Sheet sheet = getSheetByIndexId(inputStream, fileName, indexId);
        list = getRowIndex(sheet);
     ...
     * @throws IOException
     */
    public int getSheetSum(InputStream inputStream, String fileName) throws IOException {
        if (this.inputStream != null) {
            inputStream = this.inputStream;
        }
        Workbook work = this.getWorkbook(inputStream, fileName);
        return work.getNumberOfSheets();
    }
    ...
    public String getSheetName(InputStream inputStream, String fileName, int sheetIndex) throws IOException {
        if (this.inputStream != null) {
            inputStream = this.inputStream;
        }
    ...
    private Sheet getSheetByIndexId(InputStream inputStream, String fileName, int sheetIndex) throws IOException {
        if (this.inputStream != null) {
            inputStream = this.inputStream;
        }
        Workbook workbook = this.getWorkbook(inputStream, fileName);
     ...
     * @throws IOException
     */
    public Sheet getSheetIndex(int sheetIdx) throws IOException {
        Sheet sheet = null;
        // 判断文件是否存在
        ...
        try {
            // 需要判断两种格式
            if ("xls".equals(ext)) {              //使用xls方式写入
                sheet = getSheet_xls(file, sheetIdx);
            } else if ("xlsx".equals(ext)) {       //使用xlsx方式写入
                sheet = getSheet_xlsx(file, sheetIdx);
            } else {
    ...
    public String getSheetName(int sheetIdx) throws IOException {
        return getSheetIndex(sheetIdx).getSheetName();
    }
     ...
     * @return
     */
    private Sheet getSheet_xls(File file, int sheetIdx) {
        HSSFWorkbook wb = null;
        try {
            FileInputStream fis = new FileInputStream(file);
            // 去读Excel
            wb = new HSSFWorkbook(fis);
            this.sheetCount = wb.getNumberOfSheets();
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    private Sheet getSheet_xlsx(File file, int sheetIdx) {
        XSSFWorkbook wb = null;
        try {
            FileInputStream fis = new FileInputStream(file);
            // 去读Excel
    ...
    /**
     * getSheetSum 获取sheet 数量
     *
     * @return
     * @throws IOException
     * @Date
     */
    public int getSheetSum() throws IOException {
        // 判断文件是否存在
        File file = new File(this.excelPath);
     ...
     * @return
     */
    private boolean isXLS(File file) throws IOException {
        try {
            if (!file.exists()) {
                throw new IOException("文件名为" + file.getName() + "Excel文件不存在！");
            }
    ...
    public List<Row> getRowIndex(Sheet sheet) {
    ...
    public List<Row> getRowIndex(int indexIdx) throws IOException {

        Sheet sheet = getSheetIndex(indexIdx);
        return getRowIndex(sheet);
    }
     ...
     * @return
     */
    public List<Map<String, Object>> getData(List<Row> list) {
...
//                        objectList.add(body);
    ...
    public List<Map<String, Object>> readExcel1(Workbook wb) {
        List<Row> rowList = new ArrayList<Row>();
        List<Map<String, Object>> allList = new ArrayList<>();
...
//                    rowList.add(row);
                    ...
                    for (int j = 0; j < row.getLastCellNum(); j++) {
                        ...
                        if (!value.equals("")) {
                            out(value + " | ", false);
                            map.put(row.getCell(row.getFirstCellNum()).getStringCellValue(), value);
                        }
                    }
                    out("");
                }
